...
import gan08_Losses as Losses  # file
from data import get_data_loader
from models import update_average
from gan08_Blocks import DiscriminatorTop, DiscriminatorBlock, GSynthesisBlock
from gan08_CustomLayers import EqualizedConv2d, PixelNormaLayer, EqualizedLinear, Truncation

class GMapping(nn.Module):

    def __init__(self, latent_size = 512, dlatent_size = 512, dlatent_broadcast=None,
                mapping_layer=8, mapping_fmaps=512, mapping_lrmul=0.01, mapping_nonlinearity = 'lrelu',
        ...
        dlatent_broadcast : Output disentangled latent (W) as [minibatch, dlatent_size]
        ...
        mapping_lrmul : Learning rate multiplier for the mapping layers
        ...
        # Embed labels and concatenate the, with latents
        # TODO

        layers = []
        # Normalize latents
        if normalize_latents:
            layers.append(('pixel_norm', PixelNormaLayer()))    # PixelNormaLayer

        # Mapping layers. (apply_bias ?)
        layers.append(('dense0', PixelNormaLayer(self.latent_size, self.mapping_fmaps,      # PixelNormaLayer
        ...
        for layer_idx in range(1, mapping_layers):
            ...
            layers.append(
                ('dense{:d}'.format(layer_idx),
                 EqualizedLinear(fmaps_in, fmaps_out, gain=gain, lrmul = mapping_lrmul, use_wscale = use_wscale)))  # EqualizedLinear
            layers.append(('dense{:d}_act'.format(layer_idx), act))

        # Output
        self.map = nn.Sequential(OrderedDict(layers)))  # OrderedDict : 
    ...
    def forward(self, x):
        ...
        # Broadcast : 어떤 조건을 만족한다면 모양이 다른 배열끼리의 연산도 가능하게 해주며  
        #             모양이 부족한 부분은 확장하여 연산을 수행할 수 있도록 한다는 것
        if self.dlatent_broadcast is not None:
            x = x.unsequeeze(1).expand(-1, self.dlatent_broadcasat, -1)
        return x
...
class GSynthesis(nn.Module):
    ...
    def __init__(self, dlatent_size = 512, num_channels = 3, resolution = 1024,
                ...
                use_style=True, const_input_layer=True, use_noise=True, nonlinearity='lrelu',
        ...
        fmap_decay : log2 feature map reduction when doubing the resolution
        ...
        use_wscale : Enable equalized learning rate?
        use_pixel_norm : Enable pixel_wise feature vector normalization?
        use_instance_norm : Enable instance normalization?
        blur_filter : Low-pass filter to apply when resampling activations. None = no filtering
        ...
        self.structure = structure  # 'fixed' or 'linear'

        resolution_log2 = int(np.log2(resolution))
        assert resolution == 2 ** resolution_log2 and resolution >= 4   # assert만족 못할 시 error 발생
        ...
        # Early layers.
        self.init_block = InputBlock(nf(1), dlatent_size, const_input_layer, gain, use_wscale,      # InputBlock
                                     use_noise, use_pixel_norm, use_instance_norm, use_styles, act)
        # create the ToRGB layers for various outputs
        rgb_converters = [EqualizedConv2d(nf(1), num_channels, 1, gain=1, use_wscale=use_wscale)]   # EqualizedConv2d
        ...
        for res in range(3, resolution_log2 + 1):
            ...
            blocks.append(GSynthesisBlock(last_channels, channels, blur_filter, dlatent_size, gain, use_wscale, # GSynthesisBlock
                                          use_noise, use_pixel_norm, use_instance_norm, use_styles, act))
            rgb_converters.append(EqualizedConv2d(channels, num_channels, 1, gain=1, use_wscale=use_wscale))    # EqualizedConv2d
    ...
    def forward(self, latents_in, depth, alpha, labels_in=None):
        ...
        if self.training:
            ...
            if self.style_mixing_prob is not None and self.style_mixing_prob > 0:
                ...
                layer_idx = torch.from_numpy(np.arange(self.num_layers)[np.newaxis, :, np.newaxis]).to( # np.newaxis : 새로운 축 추가
                    latents_in.device)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, resolution, num_channels=3, fmap_base=8192, fmap_decay=1.0, fmap_max=512,
        ...
        for res in range(resolution_log2, 2, -1):
            # name = '{s}x{s}'.format(s=2 ** res)
            blocks.append(DiscriminatorBlock(nf(res - 1), nf(res - 2),                  # DiscriminatorBlock
                                             gain=gain, use_wscale=use_wscale, activation_layer=act,
                                             blur_kernel=blur_filter))
            # create the fromRGB layers for various inputs:
            from_rgb.append(EqualizedConv2d(num_channels, nf(res - 1), kernel_size=1,   # EqualizedConv2d
                                            gain=gain, use_wscale=use_wscale))
        self.blocks = nn.ModuleList(blocks)

        # Building the final block.
        self.final_block = DiscriminatorTop(self.mbstd_group_size, self.mbstd_num_features,         # DiscriminatorTop
                                            in_channels=nf(2), intermediate_channels=nf(2),
                                            gain=gain, use_wscale=use_wscale, activation_layer=act) # EqualizedConv2d
    ...
    def forward(self, images_in, depth, alpha=1., labels_in=None):
        ...
        if self.structure == 'fixed':
            ...
            if depth > 0:
                residual = self.from_rgb[self.depth - depth](self.temporaryDownsampler(images_in))  # temporaryDownsampler = AvgPool2d
...
class StyleGAN:
    ...
    def __setup_loss(self, loss):   # loss
    ...
    @staticmethod
    def create_grid(samples, scale_factor, img_file):   # smaple에서 격자 그리는 거
        """
