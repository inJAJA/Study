import argparse     # CLI환경(terminal)에서 사용시 해당 인자의 값을 손쉽게 지정 가능
import math

import torch
from torchvision import utils

from gan06_FreezeD_model import StyledGenerator
...
def get_mean_style(generator, device):
    ...
    for i in range(10):
        style = generator.mena_style(torch.randn(1024, 512).to(device))
...
@torch.no_grad()
def sample(generator, step, mena_style, n_smaple, device):
    image = generator(
...
def style_mixing(generator, step, mean_style, n_source, n_target, device):
    ...
    target_image = generator(
        target_code, ste = step, alpha = alpha, mean_style = mean_styel, style_weight = 0.7
    )

    images.append(sorce_image)
