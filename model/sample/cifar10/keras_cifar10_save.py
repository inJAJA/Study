...
modelpath = './model/sample/cifar10/cifar10_checkpoint_best_{epoch:02d}-{val_loss:.4f}.hdf5'
checkpoint = ModelCheckpoint(filepath = modelpath, monitor = 'val_loss',
                            save_best_only = True)

#3. fit
model.compile(loss = 'categorical_crossentropy', optimizer = 'adam', metrics = ['acc'])
model.fit(x_train, y_train, epochs= 140, batch_size = 256, callbacks = [checkpoint],
              validation_split =0.2, shuffle = True, verbose =2)


""" save_weights """
model.save_weights('./model/sample/cifar10/cifar10_save_weights.h5')
