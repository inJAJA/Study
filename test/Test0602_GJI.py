...
sm = pd.read_csv('./data/csv/samsung.csv', index_col = 0, header= 0, sep = ',',encoding = 'cp949')
h = pd.read_csv('./data/csv/hite.csv', index_col =0, header= 0, sep = ',', encoding = 'cp949')
...
print(h1[['고가','저가','종가','거래량']].mean())         # 고가    22747.834646
                                                        # 저가    22061.318898
                                                        # 종가    22415.846457
                                                        # 거래량    338999.137795
...
np.save('./test/smsung_data.npy', arr = np_sm)
np.save('./test/hite_data.npy', arr = np_h)
...
np.save('./test/x1_data5.npy', arr = x1)
np.save('./test/x2_data5.npy', arr = x2)
np.save('./test/y_data5.npy', arr = y1)
...
# model_save
model.save('./test/test0622_model5.h5')

# callbacks
# Earlystopping
es = EarlyStopping(monitor= 'val_loss', patience= 50, verbose= 1, mode = 'auto')
# Modelcheckpoint
modelpath = './test/test0622_check5_{epoch:02d}-{val_loss:.4f}.hdf5'
...
model.save_weights('./test/test0622_weights5.h5')
...
print(x1[-1,:])
a = x1_test[-1, :].reshape(1, 5, 1)
b = x2_test[-1, :].reshape(1, 5,5)
y_predict = model.predict([a, b])
print('2020.06.03 :', y_predict)

'''
loss:  1784620.849009901
mse: 1784620.875
RMSE:  1335.897027485883
R2:  0.9199679506570687
2020.06.03 : [[59336.28]]
'''