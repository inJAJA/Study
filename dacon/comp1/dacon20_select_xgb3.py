...
# np.hstack
x = np.hstack((train_rho2, train_ratio_0,  train_scr_fft, train_dst_fft, train_scr_fft_imag, train_dst_fft_imag ))
x_pred = np.hstack((test_rho2, test_ratio_0,  test_scr_fft, test_dst_fft, test_scr_fft_imag, test_dst_fft_imag))
...
for i in range(len(multi_XGB.estimators_)):
    ...
    for thres in threshold:
        ...
        xgb = XGBRegressor()
        search = RandomizedSearchCV( xgb, parameter, cv = 3)
        ...
        if mae < best_mae or score > best_socre:
            best_mae = mae
            best_score = score
            # submission
            a = np.arange(10000,20000)
            submission = pd.DataFrame(y_predict, a)
            submission.to_csv('./dacon/comp1/sub/dacon20_3_%.2f_%.5f.csv'%(score*100.0, best_mae),index = True, header=['hhb','hbo2','ca','na'],index_label='id')
